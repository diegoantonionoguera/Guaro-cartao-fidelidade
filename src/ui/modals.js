...
function renderNewClientModal() {
    return `
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg w-full max-w-md max-h-[92vh] overflow-y-auto shadow-2xl">
        
        <!-- Header -->
        <div class="relative bg-white/[0.02] p-4 sm:p-6 border-b border-white/10">
          <button
            id="btn-close-modal"
            class="absolute top-4 right-4 sm:top-5 sm:right-5 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-zinc-300 hover:text-white flex items-center justify-center motion-control cursor-pointer z-10"
          >
            ✕
          </button>
        ...
        <form id="form-new-client" class="p-6 space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            />
            <span class="text-xs text-zinc-500">Contato do cliente para atendimento.</span>
          </div>
          ...
          <div class="space-y-1.5">
            ...
            />
            <span class="text-xs text-zinc-500">O código de resgate será enviado para este endereço.</span>
          </div>
          ...
          <div class="responsive-actions pt-2 flex items-center space-x-2">
            <button
              type="button"
              id="btn-close-modal-alt"
              class="w-1/2 py-3 bg-white/5 text-zinc-300 hover:bg-white/10 font-bold rounded-xl text-xs uppercase tracking-wider motion-control cursor-pointer"
            >
              Cancelar
            </button>
            <button
              type="submit"
              class="w-1/2 py-3 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl text-xs uppercase tracking-wider shadow-xl motion-control cursor-pointer"
            >
              Cadastrar
            </button>
          </div>
...
function renderEditClientModal() {
    ...
    if (!client)
        return '';
    return `
    <div data-modal-layer data-state="entering" class="edit-client-modal modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div data-modal-panel class="edit-client-panel modal-panel bg-[#09090b] border border-white/10 rounded-lg w-full max-w-md shadow-2xl overflow-hidden">
        <div class="edit-client-header relative bg-white/[0.02] p-5 sm:p-6 border-b border-white/10">
          <button id="btn-close-modal" type="button" aria-label="Fechar" class="absolute top-4 right-4 w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 text-zinc-300 hover:text-white"><svg aria-hidden="true" class="w-4 h-4 mx-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-width="2" d="M6 6l12 12M18 6 6 18"/></svg></button>
        ...
        <form id="form-edit-client" class="edit-client-form p-5 sm:p-6">
          <input type="hidden" name="clientId" value="${escapeAttribute(client.id)}" />
          <div class="edit-client-fields">
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Nome completo</label>
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">WhatsApp / celular</label>
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">E-mail</label>
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">CPF</label>
          ...
          <div class="edit-client-balance flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-3">
            <div>
              <span class="block text-xs font-bold uppercase tracking-wider text-zinc-400">Saldo de pontos</span>
              <span class="text-xs text-zinc-500">Alterado somente por compras, resgates e estornos.</span>
            </div>
            <strong class="shrink-0 text-amber-300">${Number(client.saldoPontos) || 0} pts</strong>
          </div>
          ${store.currentUser.perfil === 'gerente' ? `
            <button type="button" data-action="delete-client" data-client-id="${escapeAttribute(client.id)}" class="edit-client-delete bg-red-500/10 hover:bg-red-500/20 border border-red-500/25 text-red-300 rounded-xl text-xs uppercase tracking-wider font-bold">
              Excluir cliente
            </button>
          ` : ''}
          <div class="edit-client-actions responsive-actions flex items-center gap-2">
...
function renderAddPointsModal() {
    ...
    return `
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg w-full max-w-lg max-h-[92vh] overflow-y-auto shadow-2xl">
        ...
        <div class="relative bg-white/[0.02] p-4 sm:p-6 border-b border-white/10">
          ...
          <div class="flex items-center space-x-2">
            <span class="text-xs uppercase font-mono tracking-widest px-2.5 py-0.5 rounded-full bg-white/10 text-white">
              Nova Compra
            </span>
          </div>
        ...
        <!-- Quota Banner -->
        <div class="px-6 pt-4">
          <div class="bg-black/40 border border-white/10 p-3 rounded-lg flex items-center justify-between text-xs font-mono">
        ...
        <form id="form-add-points" class="p-6 space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            />
            <span class="text-xs text-zinc-500 font-mono">Obrigatório para controle de auditoria e comanda.</span>
          </div>
          ...
          <!-- Points preview box -->
          <div class="p-4 bg-white/[0.02] border border-white/10 rounded-lg space-y-1">
    ...
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg w-full max-w-md max-h-[92vh] overflow-y-auto shadow-2xl">
        ...
        <div class="relative bg-white/[0.02] p-4 sm:p-6 border-b border-white/10">
          ...
          <div class="flex items-center space-x-2">
            <span class="text-xs uppercase font-mono tracking-widest px-2.5 py-0.5 rounded-full bg-amber-400/20 text-amber-400 border border-amber-400/30 font-bold">
        ...
        return `
                    ...
                    >
                      <span class="min-w-0 text-left">
                        ...
                        <span class="block mt-0.5 truncate text-xs text-zinc-400">${coupon.descricao}</span>
                      </span>
                      <span class="shrink-0 text-right">
                        <strong class="block text-sm text-amber-300">${coupon.pontosNecessarios} pts</strong>
                        <span class="block text-xs text-emerald-300">R$ ${Number(coupon.valorDescontoReais).toFixed(2)} OFF</span>
                      </span>
                    </button>
                  `;
    }).join('')}
          ...
          ${selectedCoupon ? `
            <div class="bg-black/40 border border-white/10 p-4 rounded-lg flex items-center justify-between">
              <div>
                <div class="text-xs text-zinc-400 uppercase font-bold">Cupom selecionado</div>
                <div class="text-sm font-semibold text-white">${selectedCoupon.titulo}</div>
              </div>
              <div class="text-right">
                <div class="text-xs text-zinc-400 uppercase font-bold">Desconto</div>
          ...
          ${!client.email ? `
            <div class="bg-red-950/40 border border-red-500/30 p-4 rounded-lg text-xs text-red-200 space-y-1">
          ...
          ` : `
            <div id="redemption-step-container" class="space-y-4">
              <div class="p-4 bg-white/[0.02] border border-white/10 rounded-lg text-xs text-zinc-300 space-y-2 font-sans">
                <p>Um <strong>código de 6 dígitos</strong> será enviado para <strong class="text-white">${client.email}</strong>.</p>
                <p class="text-xs text-zinc-500">O código expira em 1 minuto. O campo permanece disponível por 2 minutos.</p>
              </div>

              <button
                id="btn-generate-email-code"
                data-client-id="${client.id}"
                class="${pending ? 'hidden' : ''} w-full py-3.5 bg-gradient-to-r from-[#E32227] to-[#FF7A00] text-white font-extrabold rounded-xl text-xs uppercase tracking-wider shadow-xl motion-control cursor-pointer disabled:opacity-50"
              >
              ...
              <div id="email-verification-box" class="${pending ? '' : 'hidden'} space-y-4 pt-2 border-t border-white/10">
                ...
                <div class="flex items-center justify-between gap-3">
                  <p id="redemption-code-status" class="text-xs font-semibold text-amber-300">Código válido por 1 minuto</p>
                  <div class="text-right">
                    <span class="block text-xs uppercase tracking-wider text-zinc-500">Tempo para preencher</span>
                ...
                <button
                  ...
                  class="w-full py-3.5 bg-amber-400 hover:bg-amber-300 text-black font-extrabold rounded-xl text-xs uppercase tracking-wider shadow-xl motion-control cursor-pointer"
                >
...
function renderClientDetailsModal() {
    ...
    return `
    <div data-modal-layer data-state="entering" class="client-details-modal modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl">
        ...
        <div class="relative bg-white/[0.02] p-4 sm:p-6 border-b border-white/10">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:space-x-4 pr-8 sm:pr-0">
            <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-lg bg-white/10 border border-white/20 text-white font-black text-xl sm:text-2xl flex items-center justify-center shadow-inner shrink-0">
            ...
            <div>
              <div class="flex items-center space-x-2 flex-wrap gap-1">
                ...
                <span class="text-xs font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-white/10 text-white border border-white/20">
                  ${client.nivel}
                </span>
              </div>
        ...
        <div class="p-4 sm:p-6 space-y-6">
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
            <div class="bg-black/40 p-3 rounded-xl border border-white/10">
              <span class="text-xs text-zinc-400 uppercase">Saldo Pontos</span>
              <div class="text-lg font-black text-amber-400">${client.saldoPontos} pts</div>
            </div>
            <div class="bg-black/40 p-3 rounded-xl border border-white/10">
              <span class="text-xs text-zinc-400 uppercase">Total Acumulado</span>
              <div class="text-lg font-black text-white">${client.totalPontosAcumulados} pts</div>
            </div>
            <div class="bg-black/40 p-3 rounded-xl border border-white/10">
              <span class="text-xs text-zinc-400 uppercase">Total Resgatado</span>
              <div class="text-lg font-black text-red-400">${client.totalResgates} pts</div>
            </div>
            <div class="bg-black/40 p-3 rounded-xl border border-white/10">
              <span class="text-xs text-zinc-400 uppercase">Volume em Compras</span>
          ...
          <div class="space-y-3">
            <div class="flex items-center space-x-2 border-b border-white/10 pb-2">
              <button
                ...
                class="px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider motion-control cursor-pointer ${store.clientDetailsTab === 'lancamentos' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white'}"
              >
                Lançamentos (${txs.length})
              </button>
              <button
                id="btn-client-tab-rd"
                class="px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider motion-control cursor-pointer ${store.clientDetailsTab === 'resgates' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white'}"
              >
                Resgates (${rds.length})
              </button>
            </div>
                ...
                ` : txs.map(t => `
                  <div class="bg-white/[0.02] border border-white/10 p-3.5 rounded-xl flex items-center justify-between text-xs font-mono">
                    ...
                    <div class="text-right">
                      ...
                      <div class="text-xs uppercase font-bold text-zinc-400">${t.status}</div>
                    </div>
                  </div>
                `).join('')}
              </div>
                ...
                ` : rds.map(r => `
                  <div class="bg-white/[0.02] border border-white/10 p-3.5 rounded-xl flex items-center justify-between text-xs font-mono">
                    ...
                    <div class="text-right">
                      ...
                      <div class="text-xs uppercase font-bold text-zinc-400">${r.status}</div>
                    </div>
                  </div>
                `).join('')}
              </div>
...
function renderRejectTxModal() {
    return `
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg p-6 w-full max-w-sm space-y-4 shadow-2xl">
        ...
        <textarea
          ...
          required
        ></textarea>
        <div class="responsive-actions flex items-center space-x-2">
          <button
            type="button"
            id="btn-close-modal"
...
function renderUserModal() {
    ...
    return `
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg p-6 w-full max-w-md space-y-4 shadow-2xl">
        <div class="flex items-center justify-between gap-4">
          ...
          <button id="btn-close-modal" type="button" aria-label="Fechar" class="w-11 h-11 text-zinc-300 hover:text-white"><svg aria-hidden="true" class="w-4 h-4 mx-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-width="2" d="M6 6l12 12M18 6 6 18"/></svg></button>
        </div>

        <form id="form-user-modal" class="space-y-4">
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Nome do Usuário</label>
            <input
              type="text"
              name="nome"
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Login</label>
            <input
              type="text"
              name="login"
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">${editingUser ? 'Nova senha (opcional)' : 'Senha de acesso'}</label>
            <input
              type="password"
              name="password"
              minlength="8"
              ...
              class="w-full p-3 bg-white/[0.04] border border-white/10 rounded-xl text-sm text-white placeholder-zinc-500"
            />
            <p class="text-xs text-zinc-500">A senha será protegida antes de ser gravada na planilha.</p>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <fieldset class="space-y-1.5">
              <legend class="text-xs font-bold uppercase tracking-wider text-zinc-400 mb-1.5">Perfil</legend>
            ...
            <div class="space-y-1.5">
              <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Cota Diária</label>
              <input
                type="number"
                name="cotaDiaria"
                min="0"
          ...
          <div class="responsive-actions flex items-center gap-2">
            <button type="button" id="btn-close-modal-alt" class="flex-1 py-3 bg-white/5 text-zinc-300 hover:bg-white/10 rounded-xl text-xs uppercase tracking-wider">Cancelar</button>
    ...
    <div data-modal-layer data-state="entering" class="modal-layer fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div data-modal-panel class="modal-panel bg-[#09090b] border border-white/10 rounded-lg p-6 w-full max-w-md space-y-4 shadow-2xl">
        ...
        <form id="form-coupon-modal" class="space-y-4">
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Título</label>
            <input
              type="text"
              name="titulo"
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Descrição</label>
            <textarea
              name="descricao"
              required
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div class="space-y-1.5">
              <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Pontos Necessários</label>
              <input
                type="number"
            ...
            <div class="space-y-1.5">
              <label class="text-xs font-bold uppercase tracking-wider text-zinc-400">Valor do Desconto</label>
              <input
                type="number"
                step="0.01"
                min="0.01"
