import { store } from '../store';
import logoUrl from '../assets/brand/guaro-logo.png';
export function renderNavbar({
    animatePendingBadge = false
} = {}) {
    const currentUser = store.currentUser;
    const pendingTxs = store.transactions.filter(t => t.status === 'pendente').length;
    const remainingQuota = store.getRemainingQuotaForUser(currentUser.id);
    const transactionsToday = store.getTransactionsToday(currentUser.id);
    const totalLancadosHoje = transactionsToday.reduce((total, transaction) => total + Number(transaction.pontosGerados || 0), 0);
    const totalComprasHoje = transactionsToday.reduce((total, transaction) => total + Number(transaction.valorCompra || 0), 0);
    return `
    <header class="app-header bg-[#161616]/92 backdrop-blur-md sticky top-3 z-40">
      <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div class="mobile-nav-layout flex flex-wrap items-center justify-between min-h-16 gap-2 py-3">
          
          <!-- Logo -->
          <div class="mobile-brand flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
            <div class="header-logo-frame"><img src="${logoUrl}" alt="" class="header-logo" /></div>
            <div class="brand-copy min-w-0">
              <div class="flex items-center space-x-2">
                <span class="brand-name block text-sm sm:text-base font-black text-white font-sans" aria-label="Guaro El Buen Venezolano">
                  <span class="brand-name-full">Guaro El Buen Venezolano</span>
                  <span class="brand-name-compact" aria-hidden="true">Guaro</span>
                </span>
                <span class="hidden sm:inline-block text-xs uppercase font-bold tracking-widest px-2 py-0.5 rounded-full bg-white/10 text-zinc-300 border border-white/10">
                  Equipe
                </span>
              </div>
              <p class="text-xs text-zinc-400 hidden sm:block">Fidelidade Guaro</p>
            </div>
          </div>

          <!-- Navigation Tabs (Desktop & Mobile) -->
          <nav aria-label="Navegação principal" class="mobile-primary-nav flex items-center space-x-1 sm:space-x-1.5 bg-white/[0.03] p-1 rounded-xl border border-white/10">
            <button
              id="btn-nav-dashboard"
              class="px-2.5 sm:px-4 py-1.5 text-xs sm:text-xs font-bold uppercase tracking-wider rounded-lg motion-control flex items-center space-x-1 sm:space-x-1.5 cursor-pointer ${store.activeTab === 'dashboard'
        ? 'bg-white text-black shadow-sm'
        : 'text-zinc-400 hover:text-white hover:bg-white/5'}"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
              <span>Atendimento</span>
            </button>

            <button
              id="btn-nav-manager"
              class="px-2.5 sm:px-4 py-1.5 text-xs sm:text-xs font-bold uppercase tracking-wider rounded-lg motion-control flex items-center space-x-1 sm:space-x-1.5 relative cursor-pointer ${store.activeTab === 'manager'
        ? 'bg-white text-black shadow-sm'
        : 'text-zinc-400 hover:text-white hover:bg-white/5'}"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
              <span>Gerência</span>
              ${pendingTxs > 0 ? `
                <span data-status-badge="pending" class="w-4 h-4 rounded-full bg-amber-400 text-black text-xs font-black flex items-center justify-center ${animatePendingBadge ? 'status-badge-change' : ''}">
                  ${pendingTxs}
                </span>
              ` : ''}
            </button>
          </nav>

          <!-- Right Controls -->
          <div class="mobile-nav-actions flex flex-wrap items-center justify-end gap-1.5 sm:gap-3">
            
            <!-- Indicador dos lançamentos do dia -->
            <div class="daily-counter relative">
              <button
                id="btn-quota-toggle"
                class="daily-counter-button cursor-pointer"
                aria-expanded="${store.showQuotaTooltip ? 'true' : 'false'}"
                aria-controls="quota-summary"
                aria-label="${totalLancadosHoje} pontos lançados hoje em ${transactionsToday.length} lançamentos"
              >
                <span class="daily-counter-icon">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                </span>
                <span class="daily-counter-copy">
                  <span>Lançados hoje</span>
                  <strong>${totalLancadosHoje} pts</strong>
                </span>
                <span class="daily-counter-count">${transactionsToday.length}</span>
              </button>

              <!-- Quota Info Dropdown Tooltip -->
              ${store.showQuotaTooltip ? `
                <div id="quota-summary" role="status" class="quota-tooltip absolute right-0 mt-2 w-[85vw] max-w-xs bg-[#2E2E2E] border border-white/10 rounded-lg shadow-2xl p-4 z-50 text-xs backdrop-blur-xl">
                  <div class="flex items-center justify-between pb-2 border-b border-white/10">
                    <span class="font-bold uppercase tracking-wider text-zinc-200">Resumo de hoje</span>
                    <span class="text-xs uppercase tracking-widest bg-white/10 text-zinc-300 px-2 py-0.5 rounded-full font-mono">
                      ${currentUser.perfil}
                    </span>
                  </div>
                  <div class="py-3 space-y-2 font-mono">
                    <div class="flex justify-between text-zinc-400">
                      <span>Pontos lançados:</span>
                      <span class="text-white font-bold">${totalLancadosHoje} pts</span>
                    </div>
                    <div class="flex justify-between text-zinc-400">
                      <span>Lançamentos aprovados:</span>
                      <span class="text-white font-bold">${transactionsToday.length}</span>
                    </div>
                    <div class="flex justify-between text-zinc-400">
                      <span>Valor das compras:</span>
                      <span class="text-white font-bold">${totalComprasHoje.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                    </div>
                    <div class="flex justify-between text-zinc-400">
                      <span>Cota Total Permitida:</span>
                      <span class="text-white font-bold">${currentUser.cotaDiariaPontos} pts</span>
                    </div>
                    <div class="flex justify-between pt-2 border-t border-white/10 text-amber-400 font-bold">
                      <span>Saldo Disponível:</span>
                      <span>${remainingQuota} pts</span>
                    </div>
                  </div>
                  <p class="text-xs text-zinc-500 italic border-t border-white/10 pt-2 font-sans">
                    Lançamentos acima da cota restante exigirão aprovação prévia do gerente.
                  </p>
                </div>
              ` : ''}
            </div>

            <!-- Identidade da sessão: não permite assumir outro usuário sem autenticação -->
            <div class="mobile-profile-select max-w-[12rem] bg-white/5 border border-white/10 text-white rounded-xl px-3 py-1.5">
              <strong class="block truncate text-xs font-semibold">${currentUser.nome}</strong>
              <span class="block text-xs uppercase tracking-wider text-zinc-400">${currentUser.perfil}</span>
            </div>

            <!-- Logout Button -->
            <button
              id="btn-logout"
              class="header-logout px-2.5 sm:px-3 py-1.5 rounded-xl border border-red-500/20 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 text-xs font-bold motion-control flex items-center space-x-1 cursor-pointer"
              title="Sair do sistema"
              aria-label="Sair do sistema"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
              <span class="hidden sm:inline">Sair</span>
            </button>

          </div>

        </div>
      </div>
    </header>
  `;
}
