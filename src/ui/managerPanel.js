...
export function renderManagerPanel({
    ...
    animateTeamCardsEntrance = false
} = {}) {
    const isGerente = store.currentUser.perfil === 'gerente';
    if (!isGerente) {
        return `
      <div class="bg-red-950/40 border border-red-500/30 rounded-lg p-8 text-center space-y-4 backdrop-blur-md max-w-xl mx-auto my-12">
        <div class="w-12 h-12 rounded-lg bg-red-500/20 text-red-400 flex items-center justify-center mx-auto">
        ...
        <button
          id="btn-logout"
          class="px-5 py-2.5 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl text-xs uppercase tracking-wider motion-control cursor-pointer shadow-xl inline-flex items-center space-x-2"
        >
    ...
    <div class="space-y-6">
      ...
      <!-- Header Banner -->
      <section class="manager-hero ${animatePanelEntrance ? 'surface-enter' : ''} p-5 sm:p-7 rounded-lg flex flex-col lg:flex-row lg:items-end justify-between gap-5">
        <div>
        ...
        <!-- Tab Selection -->
        <nav aria-label="Seções da gerência" class="manager-tabs" role="tablist">
          <button
            data-manager-subtab="pendentes"
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'pendentes'}"
            tabindex="${activeSubTab === 'pendentes' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'pendentes' ? 'manager-tab-active' : ''}"
          >
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'estornos'}"
            tabindex="${activeSubTab === 'estornos' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'estornos' ? 'manager-tab-active' : ''}"
          >
            <span>Estornos</span>
          </button>
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'usuarios'}"
            tabindex="${activeSubTab === 'usuarios' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'usuarios' ? 'manager-tab-active' : ''}"
          >
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'cupons'}"
            tabindex="${activeSubTab === 'cupons' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'cupons' ? 'manager-tab-active' : ''}"
          >
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'relatorios'}"
            tabindex="${activeSubTab === 'relatorios' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'relatorios' ? 'manager-tab-active' : ''}"
          >
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'config'}"
            tabindex="${activeSubTab === 'config' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'config' ? 'manager-tab-active' : ''}"
          >
          ...
          <button
            ...
            role="tab"
            aria-controls="manager-active-panel"
            aria-selected="${activeSubTab === 'auditoria'}"
            tabindex="${activeSubTab === 'auditoria' ? '0' : '-1'}"
            class="manager-tab motion-control ${activeSubTab === 'auditoria' ? 'manager-tab-active' : ''}"
          >
            <span>Auditoria</span>
          </button>
        </nav>
      ...
      <div id="manager-active-panel" role="tabpanel" tabindex="0">
        ...
        <div class="space-y-4">
          ...
          ` : `
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ${pendingTxs.map(tx => `
                <div class="bg-[#09090b] border border-white/20 rounded-lg p-5 space-y-4 shadow-2xl relative overflow-hidden backdrop-blur-md">
                  <div class="absolute top-0 right-0 bg-white text-black text-xs font-extrabold px-3 py-1 rounded-bl-xl uppercase tracking-widest">
                    Excedente
                  </div>

                  <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-zinc-500">Atendente Solicitante</div>
                  ...
                  <div class="text-xs text-zinc-400 italic bg-white/[0.02] p-2.5 rounded-xl border border-white/5 font-mono">
                    "${tx.motivoPendente}"
                  </div>
                  ...
                  <div class="flex items-center space-x-2 pt-1">
                    <button
                      ...
                      data-tx-id="${tx.id}"
                      class="flex-1 py-2.5 bg-white/5 hover:bg-white/10 text-red-400 hover:text-red-300 font-bold rounded-xl text-xs uppercase tracking-wider border border-white/10 motion-control flex items-center justify-center space-x-1.5 cursor-pointer"
                    >
                      <span>Rejeitar</span>
                    </button>
                    ...
                    <button
                      ...
                      data-tx-id="${tx.id}"
                      class="flex-1 py-2.5 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl text-xs uppercase tracking-wider shadow-xl motion-control flex items-center justify-center space-x-1.5 cursor-pointer"
                    >
                      <span>Aprovar</span>
                    </button>
                  </div>
        ...
        <div class="space-y-6">
          <div class="space-y-3">
            ...
            ${approvedTxs.length === 0 ? `
              <div class="empty-state empty-state-compact">
                <p class="text-sm text-zinc-400">Nenhum lançamento aprovado está disponível para estorno.</p>
              </div>
            ` : `
              <div class="space-y-2">
                ${approvedTxs.map(tx => `
                  <div class="bg-[#09090b] border border-white/10 p-4 rounded-lg flex items-center justify-between text-xs backdrop-blur-md">
                    <div>
                    ...
                    <button
                      ...
                      data-tx-id="${tx.id}"
                      class="bg-red-950/60 hover:bg-red-900/80 text-red-300 border border-red-500/30 font-bold px-3 py-1.5 rounded-xl motion-control flex items-center space-x-1 text-xs uppercase tracking-wider cursor-pointer"
                    >
                      <span>Estornar</span>
                    </button>
                  </div>
          ...
          <div class="space-y-3 pt-4 border-t border-white/10">
            ...
            ${confirmedRds.length === 0 ? `
              <div class="empty-state empty-state-compact">
                <p class="text-sm text-zinc-400">Nenhum resgate confirmado está disponível para estorno.</p>
              </div>
            ` : `
              <div class="space-y-2">
                ${confirmedRds.map(rd => `
      ...
      ${activeSubTab === 'usuarios' ? `
        <div class="space-y-6">
          <div class="section-toolbar flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 sm:p-6 rounded-lg">
            <div>
            ...
            <button
              ...
              class="px-4 py-2.5 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl text-xs uppercase tracking-wider motion-control cursor-pointer shadow-lg flex items-center space-x-1.5 shrink-0"
            >
        ...
        return `
                <article class="team-card ${animateTeamCardsEntrance ? 'surface-enter' : ''} rounded-lg p-5 sm:p-6 space-y-5 flex flex-col justify-between">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <span class="text-xs font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border ${isGerente ? 'bg-amber-400/10 text-amber-300 border-amber-400/30' : 'bg-blue-400/10 text-blue-300 border-blue-400/30'}">
                      ${u.perfil}
                    </span>
                  </div>
                  ...
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      data-user-id="${u.id}"
                      class="flex-1 py-2 bg-white/5 hover:bg-white/15 text-white font-bold rounded-xl text-xs uppercase tracking-wider border border-white/10 motion-control flex items-center justify-center space-x-1 cursor-pointer"
                    >
                    ...
                    ${!isSelf ? `
                      <button
                        ...
                        data-user-id="${u.id}"
                        class="py-2 px-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold rounded-xl text-xs uppercase tracking-wider border border-red-500/20 motion-control cursor-pointer"
      ...
      ${activeSubTab === 'cupons' ? `
        <div class="space-y-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#09090b]/60 p-4 sm:p-5 rounded-lg border border-white/10 backdrop-blur-md">
            <div>
            ...
            <button
              ...
              class="px-4 py-2.5 bg-amber-400 text-black hover:bg-amber-300 font-bold rounded-xl text-xs uppercase tracking-wider motion-control cursor-pointer shadow-lg shadow-amber-400/20 flex items-center space-x-1.5 shrink-0"
            >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ${store.coupons.length === 0 ? `
              <div class="empty-state empty-state-compact md:col-span-2 lg:col-span-3">
                <h3 class="text-base text-white">Nenhum cupom cadastrado</h3>
                <p class="text-sm text-zinc-400">Crie a primeira recompensa para disponibilizá-la no atendimento.</p>
              </div>
            ` : store.coupons.map(c => `
              <div class="bg-[#09090b] border border-white/10 hover:border-white/20 rounded-lg p-5 space-y-4 shadow-xl flex flex-col justify-between relative overflow-hidden ${!c.ativo ? 'opacity-50' : ''}">
                <div>
                  <div class="flex items-start justify-between gap-2">
                    <span class="px-2.5 py-1 rounded-full text-xs font-extrabold uppercase tracking-widest border ${c.ativo ? 'bg-emerald-950 text-white border-emerald-500/30' : 'bg-zinc-800 text-white border-zinc-600'}">
                ...
                <div class="flex items-center space-x-2 pt-2 border-t border-white/5">
                  <button
                    ...
                    data-coupon-id="${c.id}"
                    class="py-2 px-3 bg-white/5 hover:bg-white/15 text-zinc-300 font-bold rounded-xl text-xs uppercase tracking-wider border border-white/10 motion-control cursor-pointer"
                  >
      ...
      ${activeSubTab === 'relatorios' ? `
        <div class="space-y-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-[#09090b] p-5 rounded-lg border border-white/10 space-y-1 backdrop-blur-md">
              <span class="text-xs font-bold uppercase tracking-widest text-zinc-400">Pontos Gerados Hoje</span>
              <div class="text-2xl font-black text-amber-400 font-mono">${totalPontosLancadosHoje} pts</div>
              <div class="text-xs text-zinc-500 font-mono">Acumulado em compras</div>
            </div>

            <div class="bg-[#09090b] p-5 rounded-lg border border-white/10 space-y-1 backdrop-blur-md">
              <span class="text-xs font-bold uppercase tracking-widest text-zinc-400">Pontos Resgatados</span>
              <div class="text-2xl font-black text-red-400 font-mono">${totalResgatesConfirmados} pts</div>
              <div class="text-xs text-zinc-500 font-mono">Descontos fornecidos</div>
            </div>

            <div class="bg-[#09090b] p-5 rounded-lg border border-white/10 space-y-1 backdrop-blur-md">
              <span class="text-xs font-bold uppercase tracking-widest text-zinc-400">Volume Fidelizado</span>
              <div class="text-2xl font-black text-emerald-400 font-mono">R$ ${totalVendasAcumuladas.toFixed(2)}</div>
              <div class="text-xs text-zinc-500 font-mono">Volume de comanda</div>
            </div>

            <div class="bg-[#09090b] p-5 rounded-lg border border-white/10 space-y-1 backdrop-blur-md">
              <span class="text-xs font-bold uppercase tracking-widest text-zinc-400">Base de Clientes</span>
              <div class="text-2xl font-black text-white font-mono">${store.clients.length} clientes</div>
              <div class="text-xs text-zinc-500 font-mono">Cadastrados no sistema</div>
            </div>

          </div>

          <!-- Recharts Bar Chart Container -->
          <div id="recharts-sales-volume-mount" class="bg-[#09090b] p-6 rounded-lg border border-white/10 backdrop-blur-md"></div>

          <div class="bg-[#09090b] p-6 rounded-lg border border-white/10 space-y-4 backdrop-blur-md">
                  ...
                  <div class="bg-black/40 p-4 rounded-xl border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 font-mono">
                    ...
                    <div class="grid grid-cols-1 min-[420px]:grid-cols-3 gap-2 min-[420px]:text-right">
                      <div class="bg-white/[0.02] p-2 rounded-lg border border-white/5">
                        <span class="text-xs text-zinc-500 uppercase block font-bold">Novas Contas</span>
                        <strong class="text-xs text-blue-400 font-bold">${clientCreatedCount} criadas</strong>
                      </div>
                      <div class="bg-white/[0.02] p-2 rounded-lg border border-white/5">
                        <span class="text-xs text-zinc-500 uppercase block font-bold">Frequentes</span>
                        <strong class="text-xs text-emerald-400 font-bold">${frequentCount} fiéis</strong>
                      </div>
                      <div class="bg-white/[0.02] p-2 rounded-lg border border-white/5">
                        <span class="text-xs text-zinc-500 uppercase block font-bold">Hoje / Cota</span>
      ...
      ${activeSubTab === 'config' ? `
        <form id="form-system-config" class="bg-[#09090b] p-6 rounded-lg border border-white/10 space-y-6 max-w-2xl backdrop-blur-md">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="text-xs font-bold uppercase tracking-widest text-zinc-400">
              ...
              />
              <span class="text-xs text-zinc-500 font-mono">Padrão: 1.0 (R$ 1 = 1 pt)</span>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                class="w-full p-3 bg-white/[0.04] border border-white/10 focus:border-white/30 rounded-xl text-white font-mono font-bold"
              />
              <span class="text-xs text-zinc-500 font-mono">Padrão: 100 pontos</span>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                class="w-full p-3 bg-white/[0.04] border border-white/10 focus:border-white/30 rounded-xl text-white font-mono font-bold"
              />
              <span class="text-xs text-zinc-500 font-mono">Padrão: R$ 10,00</span>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                class="w-full p-3 bg-white/[0.04] border border-white/10 focus:border-white/30 rounded-xl text-white font-mono font-bold"
              />
              <span class="text-xs text-zinc-500 font-mono">Padrão: 500 pts/dia</span>
            </div>
            ...
            <div class="space-y-1.5 sm:col-span-2">
              ...
              <input
                ...
                class="w-full p-3 bg-white/[0.04] border border-white/10 focus:border-white/30 rounded-xl text-white font-mono font-bold"
              />
              <span class="text-xs text-zinc-500">Tempo de segurança fixo: 1 minuto.</span>
            </div>
          </div>

          <button
            type="submit"
            class="w-full py-3 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl text-xs uppercase tracking-wider shadow-xl motion-control flex items-center justify-center space-x-2 cursor-pointer"
          >
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              <button
                ...
                class="px-3 py-2 rounded-xl bg-amber-400 text-black text-xs font-bold uppercase tracking-wider disabled:opacity-50"
              >
              ...
              <table class="w-full min-w-[600px] text-left text-xs text-zinc-300 font-mono">
                ...
                <tbody class="divide-y divide-white/5">
                  ${filteredAuditLogs.length === 0 ? `
                    <tr>
                      <td colspan="5" class="p-8 text-center text-sm text-zinc-400 font-sans">
                  ...
                  ` : filteredAuditLogs.map(log => `
                    <tr class="hover:bg-white/[0.02]">
                      ...
                      <td class="p-3">
                        <span class="px-2 py-0.5 rounded bg-white/10 border border-white/10 text-xs font-bold text-amber-400 uppercase tracking-widest">
                          ${log.acao}
                        </span>
                      </td>
